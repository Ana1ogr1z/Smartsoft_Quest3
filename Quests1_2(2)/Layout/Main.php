   ...
   private function list(): void
{
    ...
    if (!empty($rows)) {
        foreach ($rows as $row) {
            ...
            $name = htmlspecialchars($row['name_reviews'] ?? 'Аноним');
            $comment = htmlspecialchars($row['comment_reviews'] ?? 'Без комментария');
...
private function getRows(): void
{
    ...
    foreach ($rows as $row) {
        // Экранируем вывод и проверяем наличие данных
        $name = htmlspecialchars($row['name_reviews'] ?? '');
        $comment = htmlspecialchars($row['comment_reviews'] ?? '');
